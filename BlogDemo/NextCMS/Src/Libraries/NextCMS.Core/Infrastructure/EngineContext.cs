...
using XCode.Core.Configuration;
        ...
        protected static IEngine CreateEngineInstance(NextCMSConfig config)
        {
            if (config != null && !string.IsNullOrEmpty(config.EngineType))
            {
                ...
                if (!typeof(IEngine).IsAssignableFrom(engineType))
                    throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'XCode.Core.Infrastructure.IEngine' and cannot be configured in /configuration/NextCMS/engine[@engineType] for that purpose.");
