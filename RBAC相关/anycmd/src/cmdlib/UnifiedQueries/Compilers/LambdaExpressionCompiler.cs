...
namespace Anycmd.UnifiedQueries.Compilers
{
    using System;
        ...
        protected override LinqExpressions.Expression<Func<T, bool>> PerformCompile(QuerySpecification querySpecification)
        {
            ...
            while (objectStack.Count > 0)
            {
                ...
                if (item is Expression)
                {
                    ...
                    var propertyExpression = LinqExpressions.Expression.PropertyOrField(parameterExpression, e.Name);
                    var constantExpression = LinqExpressions.Expression.Constant(e.GetValue());
                    if (e.Type == DataTypes.String)
                    {
                        var methodName = e.Operator == RelationalOperators.EqualTo ? "Equals" : e.Operator.ToString();
