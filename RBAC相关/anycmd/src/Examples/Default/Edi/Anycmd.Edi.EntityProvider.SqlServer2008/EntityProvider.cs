	...
	public sealed class EntityProvider : IEntityProvider
	{
		...
		public ElementDataSchema GetElementDataSchema(ElementDescriptor element)
		{
			...
			else
			{
				...
				if (!element.Host.RdbSet.DbTableColumns.TryGetDbTableColumn(db, columnId, out column))
				{
			...
			try
			{
				...
				else
				{
					if (!command.IsDumb)
					{
						...
						{
							return new ProcessResult(new GeneralException("Id:" + command.CommandId + ",意外的影响行数" + n.ToString()));
						}
					}
		...
		public void Archive(OntologyDescriptor ontology, IArchive archive)
		{
			...
			{
				throw new GeneralException("归档库的数据库名不能与本体库相同");
			}
			// 创建归档库
		...
		private RdbDescriptor GetEntityDb(OntologyDescriptor ontology)
		{
			if (!DbDic.ContainsKey(ontology))
			{
				lock (Locker)
				{
					...
					{
						RdbDescriptor db;
						if (!ontology.Host.RdbSet.TryDb(ontology.Ontology.EntityDatabaseId, out db))
						{
							throw new GeneralException("意外的数据库Id" + ontology.Ontology.EntityDatabaseId.ToString());
						}
