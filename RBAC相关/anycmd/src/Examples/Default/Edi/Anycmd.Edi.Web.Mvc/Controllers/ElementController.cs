...
namespace Anycmd.Edi.Web.Mvc.Controllers
{
    ...
    public class ElementController : AnycmdController
    {
        ...
        public ActionResult Get(Guid? id)
        {
            ...
            return this.JsonResult(GetRequiredService<IRepository<Element, Guid>>().GetByKey(id.Value));
        }
        ...
        public ActionResult GetPlistElements(GetPlistElements input)
        {
            ...
            {
                throw new GeneralException("意外的实体类型Edi.Element");
            }
        ...
        public ActionResult AddOrUpdateElementActions()
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                {
                    var inputModel = new ElementAction(new Guid(row["Id"].ToString()))
                    {
                    ...
                    if (element != null)
                    {
                        entity = new ElementAction(inputModel.Id)
                        {
                    ...
                    {
                        entity = new ElementAction(new Guid(row["Id"].ToString()))
                        {
        ...
        public ActionResult AddOrRemoveNodes()
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    var inputModel = new NodeElementCare(new Guid(row["Id"].ToString()))
                    {
                        NodeId = new Guid(row["NodeId"].ToString()),
                        ElementId = new Guid(row["ElementId"].ToString())
                    };
                    bool isAssigned = bool.Parse(row["IsAssigned"].ToString());
                    NodeElementCare entity = GetRequiredService<IRepository<NodeElementCare, Guid>>().GetByKey(inputModel.Id);
                    bool isNew = true;
                    if (entity != null)
                    {
                        isNew = false;
                        if (!isAssigned)
                        {
                            GetRequiredService<IRepository<NodeElementCare, Guid>>().Remove(entity);
                        }
                        else
                        {
                    ...
                    {
                        entity = new NodeElementCare(inputModel.Id) { };
                    }
                    if (isAssigned)
                    {
                        ...
                        if (isNew)
                        {
                            var count = GetRequiredService<IRepository<NodeElementCare, Guid>>().AsQueryable().Count(a => a.ElementId == entity.ElementId
                                                                                                                    && a.NodeId == entity.NodeId);
                            if (count > 0)
                            {
                                throw new ValidationException("给定的节点已关心给定的本体元素，无需重复关心");
                            }
                            GetRequiredService<IRepository<NodeElementCare, Guid>>().Add(entity);
                        }
                        else
                        {
                            GetRequiredService<IRepository<NodeElementCare, Guid>>().Update(entity);
                        }
                    }
                    GetRequiredService<IRepository<NodeElementCare, Guid>>().Context.Commit();
                }
            }
