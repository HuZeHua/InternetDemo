...
namespace Anycmd.Edi.Web.Mvc.Controllers
{
    ...
    public class OntologyController : AnycmdController
    {
        ...
        public ActionResult Get(Guid? id)
        {
            ...
            return this.JsonResult(GetRequiredService<IRepository<Ontology, Guid>>().GetByKey(id.Value));
        }
        ...
        public ActionResult GetPlistOntologies(GetPlistResult input)
        {
            ...
            {
                throw new GeneralException("意外的实体类型Edi.Ontology");
            }
        ...
        {
            var data = GetRequiredService<IRepository<Ontology, Guid>>().Context.Query<InfoGroup>().FirstOrDefault(a => a.Id == id);
        ...
        {
            var data = GetRequiredService<IRepository<Ontology, Guid>>().Context.Query<Action>().FirstOrDefault(a => a.Id == id);
        ...
        {
            var data = GetRequiredService<IRepository<Ontology, Guid>>().Context.Query<Topic>().FirstOrDefault(a => a.Id == id);
        ...
        public ActionResult GetPlistInfoGroups(Guid? ontologyId, GetPlistResult input)
        {
            ...
            {
                throw new GeneralException("意外的实体类型Edi.InfoGroup");
            }
            foreach (var filter in input.Filters)
            {
                PropertyState property;
        ...
        public ActionResult UpdateOntologies()
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                {
                    var inputModel = new Ontology(new Guid(row["Id"].ToString()))
                    {
                    ...
                    Ontology entity = GetRequiredService<IRepository<Ontology, Guid>>().GetByKey(inputModel.Id);
                    if (entity != null)
                    {
                        entity.IsCataloguedEntity = inputModel.IsCataloguedEntity;
                        entity.IsLogicalDeletionEntity = inputModel.IsLogicalDeletionEntity;
                        GetRequiredService<IRepository<Ontology, Guid>>().Update(entity);
                        GetRequiredService<IRepository<Ontology, Guid>>().Context.Commit();
                        AcDomain.CommitEventBus();
                    }
                    else
                    {
                        throw new GeneralException("意外的本体");
                    }
                }
            }
