...
namespace Anycmd.Edi.Web.Mvc.Controllers
{
    ...
    public class NodeController : AnycmdController
    {
        ...
        public ActionResult Get(Guid? id)
        {
            ...
            return this.JsonResult(GetRequiredService<IRepository<Node, Guid>>().GetByKey(id.Value));
        }
        ...
        public ActionResult GetPlistNodes(GetPlistResult requestModel)
        {
            ...
            {
                throw new GeneralException("意外的实体类型Edi.Node");
            }
            foreach (var filter in requestModel.Filters)
            {
        ...
        public ActionResult GetCatalogNodesByParentId(Guid? nodeId, Guid? ontologyId, Guid? parentId)
        {
            ...
            var noos = GetRequiredService<IRepository<NodeOntologyCatalog, Guid>>().AsQueryable().Where(a => a.OntologyId == ontologyId.Value && a.NodeId == nodeId.Value).ToList<NodeOntologyCatalog>();
        ...
        public ActionResult AddOrRemoveCatalogs(Guid nodeId, Guid ontologyId, string addCatalogIds, string removeCatalogIds)
        {
            ...
            GetRequiredService<IRepository<NodeOntologyCatalog, Guid>>().Context.Commit();
        ...
        public ActionResult UpdateNodes()
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                if (state == "modified" || state == "")
                {
                    ...
                    Node entity = GetRequiredService<IRepository<Node, Guid>>().GetByKey(id);
                    if (entity != null)
                    {
                    ...
                    {
                        throw new GeneralException("意外的节点");
                    }
                }
            }
        ...
        public ActionResult AddOrRemoveOntologies()
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                if (state == "modified" || state == "") //更新：_state为空或modified
                {
                    ...
                    NodeOntologyCare entity = GetRequiredService<IRepository<NodeOntologyCare, Guid>>().GetByKey(id);
                    if (entity != null)
                    {
                        if (!isAssigned)
                        {
        ...
        public ActionResult AddOrRemoveElementCares()
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                {
                    bool isAssigned = bool.Parse(row["IsAssigned"].ToString());
                    NodeElementCare entity = GetRequiredService<IRepository<NodeElementCare, Guid>>().GetByKey(id);
                    if (entity != null)
                    {
                        if (!isAssigned)
                        {
        ...
        public ActionResult AddOrUpdateNodeElementActions()
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                if (state == "modified" || state == "")
                {
                    var inputModel = new NodeElementAction(new Guid(row["Id"].ToString()))
                    {
                        NodeId = new Guid(row["NodeId"].ToString()),
                        ElementId = new Guid(row["ElementId"].ToString()),
                        ActionId = new Guid(row["ActionId"].ToString()),
                    ...
                    NodeElementAction entity = GetRequiredService<IRepository<NodeElementAction, Guid>>().GetByKey(inputModel.Id);
                    if (entity != null)
                    {
                        entity.IsAudit = inputModel.IsAudit;
                        entity.IsAllowed = inputModel.IsAllowed;
                        GetRequiredService<IRepository<NodeElementAction, Guid>>().Update(entity);
                    }
                    else
                    {
                        entity = new NodeElementAction(inputModel.Id);
                        entity.NodeId = inputModel.NodeId;
                        entity.ElementId = inputModel.ElementId;
                        entity.ActionId = inputModel.ActionId;
                        entity.IsAudit = inputModel.IsAudit;
                        entity.IsAllowed = inputModel.IsAllowed;
                        var count = GetRequiredService<IRepository<NodeElementAction, Guid>>().AsQueryable().Count(a => a.NodeId == entity.NodeId
                        ...
                        GetRequiredService<IRepository<NodeElementAction, Guid>>().Add(entity);
                    }
                    GetRequiredService<IRepository<NodeElementAction, Guid>>().Context.Commit();
                    AcDomain.CommitEventBus();
                }
            }
