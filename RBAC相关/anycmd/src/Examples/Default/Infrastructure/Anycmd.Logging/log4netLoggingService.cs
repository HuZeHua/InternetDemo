...
namespace Anycmd.Logging
{
    ...
    public sealed class Log4NetLoggingService : ILoggingService
    {
        ...
        public void Log(IAnyLog[] anyLogs)
        {
            ...
            {
                throw new GeneralException("意外的数据库表标识" + tableId);
            }
            // 当前命令表模式克隆得到的新表
...
+ filterString + ") a WHERE a.RowNumber > {4}";
        ...
        private RdbDescriptor GetAnyLogDb()
        {
            ...
            {
                throw new Exceptions.GeneralException("意外的实体类型码Ac.AnyLog");
            }
            RdbDescriptor db;
            if (!_acDomain.RdbSet.TryDb(entityType.DatabaseId, out db))
            {
                throw new Exceptions.GeneralException("意外的AnyLog数据库标识" + entityType.DatabaseId);
            }
            return db;
        }
        ...
        private RdbDescriptor GetOperationLogDb()
        {
            ...
            {
                throw new Exceptions.GeneralException("意外的实体类型码Ac.OperationLog");
            }
            RdbDescriptor db;
            if (!_acDomain.RdbSet.TryDb(entityType.DatabaseId, out db))
            {
                throw new Exceptions.GeneralException("意外的OperationLog数据库标识" + entityType.DatabaseId);
            }
            return db;
        }
        ...
        private RdbDescriptor GetExceptionLogDb()
        {
            ...
            {
                throw new Exceptions.GeneralException("意外的实体类型码Ac.ExceptionLog");
            }
            RdbDescriptor db;
            if (!_acDomain.RdbSet.TryDb(entityType.DatabaseId, out db))
            {
                throw new Exceptions.GeneralException("意外的ExceptionLog数据库标识" + entityType.DatabaseId);
            }
            return db;
        }
