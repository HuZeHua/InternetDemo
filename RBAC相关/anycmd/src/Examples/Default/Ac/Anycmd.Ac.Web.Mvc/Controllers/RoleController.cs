...
namespace Anycmd.Ac.Web.Mvc.Controllers
{
    ...
    public class RoleController : AnycmdController
    {
        ...
        public ActionResult GetPlistAccountRoles(GetPlistAccountRoles requestData)
        {
            ...
            var accountRoles = GetRequiredService<IRepository<Privilege, Guid>>().AsQueryable().Where(a => a.SubjectInstanceId == requestData.AccountId && a.ObjectType == privilegeType);
            if (requestData.IsAssigned.HasValue)
            {
                if (requestData.IsAssigned.Value)
                {
                    foreach (var ar in accountRoles)
                    {
                        ...
                        {
                            throw new GeneralException("意外的角色标识" + ar.ObjectInstanceId);
                        }
            ...
            if (requestData.IsAssigned.HasValue)
            {
                if (requestData.IsAssigned.Value)
                {
                    foreach (var ar in ssdSetRoles)
                    {
                        ...
                        {
                            throw new GeneralException("意外的角色标识" + ar.RoleId);
                        }
        ...
        public ActionResult Delete(string id)
        {
            ...
            {
                AcDomain.Handle(new RemoveRoleCommand(AcSession, item));
            }
        ...
        public ActionResult GrantOrDenyRoleFunctions()
        {
            ...
            foreach (Hashtable row in rows)
            {
                ...
                if (state == "modified" || state == "")
                {
                    ...
                    Privilege entity = GetRequiredService<IRepository<Privilege, Guid>>().GetByKey(id);
                    if (entity != null)
                    {
                        if (!isAssigned)
                        {
