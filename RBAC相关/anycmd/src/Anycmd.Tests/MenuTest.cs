...
namespace Anycmd.Tests
{
    ...
    public class MenuTest
    {
        ...
        public void MenuCanNotRemoveWhenItHasChildMenus()
        {
            ...
            Assert.IsNotNull(acDomain.RetrieveRequiredService<IRepository<Menu, Guid>>().GetByKey(entityId));
            Assert.IsNotNull(acDomain.RetrieveRequiredService<IRepository<Menu, Guid>>().GetByKey(entityId2));
            Assert.AreEqual(entityId, acDomain.RetrieveRequiredService<IRepository<Menu, Guid>>().GetByKey(entityId2).ParentId.Value);
            Assert.IsTrue(acDomain.MenuSet.TryGetMenu(entityId, out menuById));
            bool catched = false;
            try
            {
        ...
        public void MenuSetShouldRollbackedWhenPersistFailed()
        {
            ...
            acDomain.RemoveService(typeof(IRepository<Menu, Guid>));
            var moMenuRepository = acDomain.GetMoqRepository<Menu, IRepository<Menu, Guid>>();
            ...
            moMenuRepository.Setup<Menu>(a => a.GetByKey(entityId1)).Returns(new Menu(entityId1) { Name = name });
            moMenuRepository.Setup<Menu>(a => a.GetByKey(entityId2)).Returns(new Menu(entityId2) { Name = name });
            acDomain.AddService(typeof(IRepository<Menu, Guid>), moMenuRepository.Object);
