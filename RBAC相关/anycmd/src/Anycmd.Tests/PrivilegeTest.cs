...
namespace Anycmd.Tests
{
    ...
    public class PrivilegeTest
    {
        ...
        public void AccountSubjectTypePrivilege()
        {
            ...
            acDomain.RetrieveRequiredService<IRepository<Account, Guid>>().Add(new Account(accountId)
            {
                Code = "test",
                Name = "test",
                Password = "111111"
            ...
            var privilegeBigram = acDomain.RetrieveRequiredService<IRepository<Privilege, Guid>>().AsQueryable().FirstOrDefault(a => a.Id == entityId);
            ...
            Assert.AreEqual(0, acDomain.PrivilegeSet.Count());// 主体为账户的权限记录不驻留在内存中所以为0
            var firstOrDefault = acDomain.RetrieveRequiredService<IRepository<Privilege, Guid>>().AsQueryable().FirstOrDefault(a => a.Id == entityId);
            if (
                firstOrDefault != null)
            ...
            Assert.IsNull(acDomain.RetrieveRequiredService<IRepository<Privilege, Guid>>().AsQueryable().FirstOrDefault(a => a.Id == entityId));
        }
        #endregion
        ...
        public void RoleSubjectTypePrivilege()
        {
            ...
            Assert.IsNotNull(privilegeBigram);
            Assert.IsNotNull(acDomain.RetrieveRequiredService<IRepository<Privilege, Guid>>().AsQueryable().FirstOrDefault(a => a.Id == entityId));
