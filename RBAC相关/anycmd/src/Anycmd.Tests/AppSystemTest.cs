...
namespace Anycmd.Tests
{
    ...
    public class AppSystemTest
    {
        ...
        public void AppSystemSet()
        {
            ...
            Assert.IsNotNull(acDomain.RetrieveRequiredService<IRepository<AppSystem, Guid>>().GetByKey(entityId));
        ...
        public void AppSystemSetShouldRollbackedWhenPersistFailed()
        {
            ...
            var moAppSystemRepository = acDomain.GetMoqRepository<AppSystem, IRepository<AppSystem, Guid>>();
            var entityId1 = Guid.NewGuid();
            var entityId2 = Guid.NewGuid();
            const string code = "app1";
            const string name = "测试1";
            acDomain.RemoveService(typeof(IRepository<AppSystem, Guid>));
            ...
            moAppSystemRepository.Setup<AppSystem>(a => a.GetByKey(entityId1)).Returns(new AppSystem(entityId1) { Code = code, Name = name });
            moAppSystemRepository.Setup<AppSystem>(a => a.GetByKey(entityId2)).Returns(new AppSystem(entityId2) { Code = code, Name = name });
            acDomain.AddService(typeof(IRepository<AppSystem, Guid>), moAppSystemRepository.Object);
