...
namespace Anycmd.Tests
{
    ...
    public class FunctionSetTest
    {
        ...
        public void FunctionSetShouldRollbackedWhenPersistFailed()
        {
            ...
            acDomain.RemoveService(typeof(IRepository<Function, Guid>));
            var moFunctionRepository = acDomain.GetMoqRepository<Function, IRepository<Function, Guid>>();
            ...
            moFunctionRepository.Setup<Function>(a => a.GetByKey(entityId1)).Returns(new Function(entityId1)
            {
                ResourceTypeId = TestHelper.TestCatalogNodeId,
                DeveloperId = acDomain.SysUserSet.GetDevAccounts().First().Id
            });
            moFunctionRepository.Setup<Function>(a => a.GetByKey(entityId2)).Returns(new Function(entityId2)
            {
                ResourceTypeId = TestHelper.TestCatalogNodeId,
                DeveloperId = acDomain.SysUserSet.GetDevAccounts().First().Id
            });
            acDomain.AddService(typeof(IRepository<Function, Guid>), moFunctionRepository.Object);
