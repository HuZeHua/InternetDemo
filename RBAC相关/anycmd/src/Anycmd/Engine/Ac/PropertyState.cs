...
namespace Anycmd.Engine.Ac
{
    ...
    public sealed class PropertyState : StateObject<PropertyState>, IProperty, IStateObject
    {
        ...
        internal PropertyState InternalModify(PropertyBase property)
        {
            ...
            {
                throw new GeneralException("实体属性必须属于某个实体类型");
            }
            EntityTypeState entityType;
            if (!_acDomain.EntityTypeSet.TryGetEntityType(property.EntityTypeId, out entityType))
            {
                throw new GeneralException("意外的实体类型标识" + property.EntityTypeId);
            }
        ...
        private EntityTypeState EntityType
        {
            get
            {
                if (_entityType == EntityTypeState.Empty)
                {
                    ...
                    {
                        throw new GeneralException("意外的实体类型标识" + this.EntityTypeId);
                    }
                }
                return _entityType;
            }
        ...
        private RdbDescriptor Database
        {
            get
            {
                if (_database == null)
                {
                    if (!AcDomain.RdbSet.TryDb(EntityType.DatabaseId, out _database))
                    {
                        throw new GeneralException("意外的数据库标识" + EntityType.DatabaseId);
                    }
                }
                return _database;
            }
        ...
        private DbTableColumn TableColumn
        {
            get
            {
                if (_tableColumn == null)
                {
                    ...
                    AcDomain.RdbSet.DbTableColumns.TryGetDbTableColumn(Database, string.Format("[{0}][{1}][{2}]", EntityType.SchemaName, EntityType.TableName, this.Code), out _tableColumn);
                }
