...
namespace Anycmd.Engine.Rdb
{
    ...
    public sealed class RdbDescriptor
    {
        ...
        public RdbDescriptor(IAcDomain acDomain, IRDatabase database)
        {
            ...
            {
                throw new GeneralException("意外的关系数据库类型" + database.RdbmsType);
            }
        ...
        {
            get { return _acDomain.RdbSet.DbTables[this]; }
        }

        public IReadOnlyDictionary<string, DbView> DbViews
        {
            get { return _acDomain.RdbSet.DbViews[this]; }
        }
...
@"data source={0};initial catalog={1};user id={2};password={3};{4}"
            ...
            get
            {
                if (!_isLocalhostDetected || !string.Equals(_dataSource, Database.DataSource, StringComparison.OrdinalIgnoreCase))
                {
                    lock (_thisLocker)
                    {
                        ...
                        {
                            throw new GeneralException("数据源为空");
                        }
                        if (_dataSource == "."
        ...
        {
            return _acDomain.RdbSet.DbTables.TryGetDbTable(this, dbTableId, out dbTable);
        }

        public bool TryGetDbView(string dbViewId, out DbView dbView)
        {
            return _acDomain.RdbSet.DbViews.TryGetDbView(this, dbViewId, out dbView);
        }
        ...
        private DataTable GetTableSchema(DbTable dbTable)
        {
            ...
            if (!_tableSchemas.ContainsKey(dbTable.Id))
            {
                lock (GlobalLocker)
                {
                    ...
                    if (!_acDomain.RdbSet.DbTableColumns.TryGetDbTableColumns(this, dbTable, out dbTableColumns))
                    {
                        throw new GeneralException("意外的数据库表");
                    }
