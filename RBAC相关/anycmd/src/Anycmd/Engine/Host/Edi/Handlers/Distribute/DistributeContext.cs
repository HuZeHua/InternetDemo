        ...
        public NodeDescriptor ClientAgent
        {
            get
            {
                if (_clientAgent == null)
                {
                    ...
                    {
                        throw new GeneralException("意外的客户端类型" + this.Command.ClientType.ToString());
                    }
                    switch (this.Command.ClientType)
                    {
                        case ClientType.Undefined:
                            throw new GeneralException("意外的客户端类型");
                        case ClientType.Node:
                            NodeDescriptor node;
                            if (!Ontology.Host.NodeHost.Nodes.TryGetNodeById(this.Command.ClientId, out node))
                            {
                                throw new GeneralException("意外的请求节点标识" + this.Command.ClientId);
                            }
                            _clientAgent = node;
                            break;
                        case ClientType.App:
