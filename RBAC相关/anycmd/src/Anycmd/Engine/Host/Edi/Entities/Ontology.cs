...
namespace Anycmd.Engine.Host.Edi.Entities
{
    ...
    /// 其次，实体上有众多属性，如教师的性别、出生日期、身份证件号、所属目录等，这类属性我们称为“元素（Element）”
    ...
    /// </remarks>
    /// </summary>
    public class Ontology : OntologyBase, IAggregateRoot<Guid>
    {
        private Ontology() { }

        public Ontology(Guid id) : base(id) { }

        public static Ontology Create(IOntologyCreateIo input)
        {
            Debug.Assert(input.Id != null, "input.Id != null");
            return new Ontology(input.Id.Value)
            {
                CanAction = false,
                CanCommand = false,
                CanEvent = false,
