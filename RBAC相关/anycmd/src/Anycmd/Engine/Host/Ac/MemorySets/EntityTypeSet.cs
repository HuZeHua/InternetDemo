    ...
    internal sealed class EntityTypeSet : IEntityTypeSet, IMemorySet
    {
        ...
        private void Init()
        {
            ...
            lock (Locker)
            {
                ...
                foreach (var entityType in entityTypes)
                {
                    ...
                    {
                        throw new GeneralException("意外的重复的实体类型标识" + entityType.Id);
                    }
                    ...
                    {
                        throw new GeneralException("意外的重复的实体类型编码" + entityType.Codespace + "." + entityType.Code);
                    }
            ...
            IHandler<RemoveEntityTypeCommand>
        {
            ...
            private void Handle(IAcSession acSession, IEntityTypeCreateIo input, bool isCommand)
            {
                ...
                var entityTypeRepository = acDomain.RetrieveRequiredService<IRepository<EntityType, Guid>>();
                ...
                lock (Locker)
                {
                    ...
                    {
                        throw new GeneralException("重复的实体类型标识" + input.Id);
                    }
        ...
        private class PropertySet : IEnumerable<PropertyState>
        {
            ...
            public bool TryGetProperty(EntityTypeState entityType, string propertyCode, out PropertyState property)
            {
                ...
                if (string.IsNullOrEmpty(propertyCode))
                {
                    throw new GeneralException("属性编码为空");
                }
            ...
            private void Init()
            {
                ...
                lock (PropertyLocker)
                {
                    ...
                    foreach (var property in properties)
                    {
                        ...
                        {
                            throw new GeneralException("意外的实体属性类型标识" + property.EntityTypeId);
                        }
                        ...
                        {
                            throw new GeneralException("意外的重复实体属性标识" + property.Id);
                        }
                ...
                IHandler<EntityTypeRemovedEvent>
            {
                ...
                public void Handle(EntityTypeUpdatedEvent message)
                {
                    ...
                    {
                        throw new GeneralException("意外的实体类型标识" + message.Source.Id);
                    }
                ...
                private void Handle(IAcSession acSession, IPropertyCreateIo input, bool isCommand)
                {
                    ...
                    var dicByCode = _set._dicByCode;
                    var dicById = _set._dicById;
                    var propertyRepository = acDomain.RetrieveRequiredService<IRepository<Property, Guid>>();
                    if (string.IsNullOrEmpty(input.Code))
                    {
                        throw new ValidationException("编码不能为空");
                    }
                    ...
                    {
                        throw new GeneralException("记录已经存在" + input.EntityTypeId);
                    }
                    Property entity;
                    lock (PropertyLocker)
                    {
                        ...
                        {
                            throw new GeneralException("记录已经存在");
                        }
                ...
                private void Update(PropertyState state)
                {
                    ...
                    {
                        throw new GeneralException("意外的实体属性类型标识" + oldState.EntityTypeId);
                    }
                    string oldKey = oldState.Code;
                    string newKey = state.Code;
                    dicById[state.Id] = state;
                ...
                private void Handle(IAcSession acSession, Guid propertyId, bool isCommand)
                {
                    ...
                    lock (PropertyLocker)
                    {
                        ...
                        if (!acDomain.EntityTypeSet.TryGetEntityType(bkState.EntityTypeId, out entityType))
                        {
                            throw new GeneralException("意外的实体属性类型标识" + bkState.EntityTypeId);
                        }
                        if (dicById.ContainsKey(bkState.Id))
                        {
