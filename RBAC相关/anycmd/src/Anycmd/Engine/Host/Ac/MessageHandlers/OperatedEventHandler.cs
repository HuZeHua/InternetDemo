...
namespace Anycmd.Engine.Host.Ac.MessageHandlers
{
    ...
    /// 操作事件处理程序
    /// </summary>
    public class OperatedEventHandler : IDomainEventHandler<OperatedEvent, Guid>
    {
        ...
        public void Handle(OperatedEvent evnt)
        {
            ...
            {
                throw new GeneralException("为设定操作日志的标识");
            }
            ...
            RdbDescriptor db;
            if (!_acDomain.RdbSet.TryDb(_operationLogDbId, out db))
            {
                throw new GeneralException("意外的数据库标识");
            }
            // TODO:logbuffer
